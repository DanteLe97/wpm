...
class MAC_Importer_Export_API {
    ...
    public static function export_elementor_data_by_url($request) {
        ...
        // Collect tất cả templates từ tất cả pages và loại bỏ duplicates
        $all_templates = array();
        $template_ids_seen = array();
        
        foreach ($exported_pages as $index => $page_data) {
            if (isset($page_data['templates']) && is_array($page_data['templates'])) {
                foreach ($page_data['templates'] as $template) {
                    $template_id = isset($template['template_id']) ? $template['template_id'] : null;
                    if ($template_id && !isset($template_ids_seen[$template_id])) {
                        $all_templates[] = $template;
                        $template_ids_seen[$template_id] = true;
                    }
                }
                // Xóa templates khỏi page data (sẽ đưa lên level data)
                unset($exported_pages[$index]['templates']);
            }
        }
        
        // Luôn trả về cùng một format: data.pages (mảng), data.templates (mảng), kèm errors nếu có
    ...
    public static function parse_url_and_get_page_data($url) {
        ...
        // Scan và lấy template IDs từ page content
        $template_ids = self::extract_template_ids_from_content($decoded_content);
        
        // Lấy template data cho mỗi template ID
        $templates = array();
        foreach ($template_ids as $template_id) {
            $template_data = self::get_template_export_data($template_id);
            if ($template_data) {
        ...
        return array(
            ...
            'templates' => $templates // Thêm templates vào page data
        );
    }
    ...
    /**
     * Extract template IDs từ Elementor content
     * Recursively scan để tìm tất cả template IDs được sử dụng trong mac-menu widgets
     */
    private static function extract_template_ids_from_content($elements) {
        $template_ids = array();
        ...
        foreach ($elements as $element) {
            // Kiểm tra nếu là mac-menu widget
            if (isset($element['widgetType']) && $element['widgetType'] === 'module_mac_menu') {
                $settings = isset($element['settings']) ? $element['settings'] : array();
                
                // Chỉ lấy template ID nếu:
                // 1. Custom layout được bật
                // 2. id_section_category không phải 'default'
                // 3. id_section_category là số (template ID)
                if (
                    isset($settings['cat_menu_is_custom_layout']) &&
                    $settings['cat_menu_is_custom_layout'] === 'on' &&
                    isset($settings['id_section_category']) &&
                    !empty($settings['id_section_category']) &&
                    $settings['id_section_category'] !== 'default' &&
                    is_numeric($settings['id_section_category'])
                ) {
                    $template_id = intval($settings['id_section_category']);
            ...
            if (isset($element['elements']) && is_array($element['elements'])) {
                $child_template_ids = self::extract_template_ids_from_content($element['elements']);
                $template_ids = array_merge($template_ids, $child_template_ids);
            }
        }
    ...
    /**
     * Lấy template export data từ template ID
     * Format chuẩn Elementor: content, page_settings, version, title, type
     */
    private static function get_template_export_data($template_id) {
        ...
        // Lấy Elementor data của template
        $template_elementor_data = get_post_meta($template_id, '_elementor_data', true);
        $template_page_settings = get_post_meta($template_id, '_elementor_page_settings', true);
        ...
        $template_content = is_string($template_elementor_data) 
            ? json_decode($template_elementor_data, true) 
            : $template_elementor_data;
        ...
        // Format chuẩn Elementor
        return array(
            'template_id' => $template_id,
            'template_name' => $template_post->post_title,
            'template_slug' => $template_post->post_name,
            'content' => $template_content,
            'page_settings' => is_array($template_page_settings) ? $template_page_settings : array(),
            'version' => '0.4',
