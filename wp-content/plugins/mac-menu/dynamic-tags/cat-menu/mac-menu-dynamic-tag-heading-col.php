 ...
 * @since 1.0.0
 */
class Elementor_Dynamic_Tag_Mac_Menu_Heading_Col extends \Elementor\Core\DynamicTags\Tag {
	...
	public function get_title() {
		return esc_html__( 'Heading Col', 'mac-plugin' );
	}
	...
	public function render() {
		...
		$headingArray = [];
		if(isset($Cat[0]->data_table) && !empty($Cat[0]->data_table) ):
			$headingArray = !empty( $Cat[0]->data_table) ? json_decode($Cat[0]->data_table, true) : [];
			if(!empty($headingArray)):
				foreach($headingArray as $item){
					if($item != ''):
