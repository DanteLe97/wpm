 ...
 * @since 1.0.0
 */
class Elementor_Dynamic_Tag_Mac_Menu_Item_Name extends \Elementor\Core\DynamicTags\Tag {
	...
	public function get_title() {
		return esc_html__( 'Item Name', 'mac-plugin' );
	}
	...
	public function render() {
		...
		$price_in_name_category_item = !empty($this->get_settings( 'price_in_name_category_item' )) ? $this->get_settings( 'price_in_name_category_item' ) :"";
		...
		if($index !=''):
			...
			foreach($Array as $key){
				if($index == $i ):
					//echo wp_kses_post( $key['name'] .'<span class="item-price" >'.$key['price'].'</span>' );
					echo wp_kses_post( $key['name'] ); 
					if(isset($key['price-list']) && !empty($key['price-list']) && !empty($price_in_name_category_item)  ):
