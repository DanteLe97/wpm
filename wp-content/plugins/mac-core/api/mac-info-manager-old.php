                            ...
                            default:
                                ...
                                switch ($operator) {
                                    ...
                                    case '<>':
                                        ...
                                        $message = $result ? 'Value khác expected' : 'Value bằng expected';
                                        break;
    ...
    private function handle_sitecheck($item) {
        try {
            ...
            if (isset($value['check_type']) && $value['check_type'] === 'security_status') {
                ...
                // Custom checks based on value parameter
            ...
            if (isset($value['check_type']) && $value['check_type'] === 'security_status') {
                ...
                $overall_result = isset($results['has_correct_logo']) ? $results['has_correct_logo'] : false;
                ...
                if ($overall_result) {
                    $overall_message = 'Tất cả logo có link đúng (' . ($results['correct_links'] ?? 0) . '/' . ($results['logo_found'] ?? 0) . ') tại page ' . $actual_page;
                } else {
                    $overall_message = 'Có logo không có link hoặc link không đúng (' . ($results['incorrect_links'] ?? 0) . '/' . ($results['logo_found'] ?? 0) . ') tại page ' . $actual_page;
                }
            } elseif (isset($value['check_type']) && $value['check_type'] === 'book_now_button') {
