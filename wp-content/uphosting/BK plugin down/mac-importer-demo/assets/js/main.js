...
(function($){
    ...
    BKADMINPANEL.documentOnReady = {
        ...
        checkExternalImages: function(pageId) {
            ...
            console.log('mac_ajax_nonce:', typeof mac_ajax_nonce !== 'undefined' ? mac_ajax_nonce : 'undefined');
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (pageId === 'all') {
                        html += 'Found <strong>' + response.data.total + '</strong> external images across all pages';
                    } else {
                        html += 'Found <strong>' + response.data.total + '</strong> external images';
                    }
                ...
                error: function() {
                    $('#download-progress').hide();
                    $('#download-results').show();
                    $('#download-results-content').html('<div class="notice notice-error"><p>Error checking images</p></div>');
                }
            });
        },
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (pageId === 'all') {
                        ...
                        html += 'Pages processed: <strong>' + response.data.pages_processed + '</strong><br>';
                        html += 'Downloaded: <strong>' + response.data.total_downloaded + '</strong> images<br>';
                        html += 'Failed: <strong>' + response.data.total_failed + '</strong> images<br><br>';
                        ...
                        html += 'Downloaded: ' + response.data.downloaded + ' images<br>';
                        html += 'Failed: ' + response.data.failed + ' images<br>';
                        html += 'Total: ' + response.data.total + ' images';
                    }
                    html += '</p></div>';
                ...
                error: function() {
                    ...
                    $('#download-progress').hide();
                    $('#download-results').show();
                    $('#download-results-content').html('<div class="notice notice-error"><p>Error downloading images</p></div>');
                }
            });
        }
    }
