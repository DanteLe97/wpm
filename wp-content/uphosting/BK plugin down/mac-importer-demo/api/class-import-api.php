...
class MAC_Importer_Import_API {
    ...
    public static function import_pages_from_domain($request) {
        ...
        if (!empty($export_response) && isset($export_response['data'])) {
            $site_settings = isset($export_response['data']['site_settings']) ? $export_response['data']['site_settings'] : $site_settings;
            if (isset($export_response['data']['page'])) {
    ...
    public static function import_site_settings_from_data($site_settings) {
        ...
        try {
            ...
            if (isset($settings['custom_colors'])) {
                update_post_meta($kit_id, '_elementor_custom_colors', $settings['custom_colors']);
            } else {
                // Fallback: Sử dụng custom_colors mặc định nếu không có trong settings
    ...
    public static function apply_elementor_template_to_page($data, $page_id) {
        ...
        try {
            ...
            // Lưu elementor data vào page
            $result = update_post_meta($page_id, '_elementor_data', $processed_data);
    ...
    private static function download_and_attach_image($image_url, $page_id) {
        ...
        $attachment_id = media_handle_sideload($file_array, $page_id, 'Imported from external URL: ' . $image_url);
        ...
        // Lưu original URL để tránh duplicate
    ...
    public static function download_all_external_images_single($page_id, $downloaded_urls = array()) {
        ...
        if ($downloaded_count > 0) {
            // Nếu elementor_data gốc là array, lưu lại array
            if (is_array($elementor_data)) {
                update_post_meta($page_id, '_elementor_data', $elements);
            } else {
                // Nếu elementor_data gốc là string, encode lại thành JSON
                update_post_meta($page_id, '_elementor_data', json_encode($elements));
            }
    ...
    /**
     * Replace URL trong settings (bao gồm background_image fields)
     */
    private static function replace_url_in_settings_complete($settings, $old_url, $new_url, $page_id) {
        ...
        // Danh sách tất cả background_image fields
        $bg_image_fields = array(
            'background_image',
            'background_overlay_image',
            'background_hover_image',
        ...
        foreach ($bg_image_fields as $field) {
            if (isset($settings[$field]) && is_array($settings[$field])) {
                $bg_image = &$settings[$field];
                if (!empty($bg_image['url']) && $bg_image['url'] === $old_url) {
                    // Replace URL
                    $bg_image['url'] = $new_url;
                    ...
                    error_log("REPLACED: Background image field '{$field}' URL: {$old_url} → {$new_url}");
                }
            }
        }
